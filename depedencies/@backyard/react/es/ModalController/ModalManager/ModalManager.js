...
class ModalManager {
  ...
  add(modal, container) {
    ...
    const containerIndex = findIndexOf(this.containers, item => item.container === container); // If container index is found,
  ...
  mount(modal, props) {
    ...
    const containerInfo = this.containers[containerIndex]; // If not restoring container,
  ...
  remove(modal) {
    // Get modal index
    const modalIndex = this.modals.indexOf(modal); // If modal index not found,
    ...
    const containerIndex = findIndexOf(this.containers, item => item.modals.indexOf(modal) !== -1);
    const containerInfo = this.containers[containerIndex]; // Remove modal from container
