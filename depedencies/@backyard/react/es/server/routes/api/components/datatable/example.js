...
module.exports = {
  ...
  handler: (request, h) => {
    ...
    const params = {
      ...
      filters: reduce(query, (filters, value, key) => {
      ...
      sorts: reduce(query, (filters, value, key) => {
    ...
    const applyGlobalFilter = (data, globalFilter) => globalFilter ? filter(data, row => some(values(row), cell => includes(toLower(cell), toLower(globalFilter)))) : data;

    const applyFilters = (data, filters) => keys(filters).length > 0 ? filter(data, row => every(entries(filters), ([key, value]) => includes(toLower(row[key]), toLower(value)))) : data;

    const applySorts = (data, sorts) => keys(sorts).length > 0 ? orderBy(data, ...reduce(entries(sorts), (sorts, [key, value]) => {
      sorts[0].push(key);
      sorts[1].push(value);
      return sorts;
    }, [[], []])) : data;

    const rows = applySorts(applyFilters(applyGlobalFilter(dataset, params.globalFilter), params.filters), params.sorts);
    const table = slice(rows, mods.start, mods.end);
    const info = {
      count: rows.length,
