...
var TimePickerNative = /*#__PURE__*/React.forwardRef(function TimePickerNative(_ref, ref) {
  ...
  var shape = getShape(shapeProp, shapeContext); // Hold the date adapter

  var adapter = useAdapter(); // Calculate if parsable date is date range

  var isRangeList = Boolean(times && isParsableDateRange(times[0]) && times[0].start); // Parse time from time prop
