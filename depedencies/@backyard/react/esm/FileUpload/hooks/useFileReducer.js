...
var useFileReducer = function useFileReducer(_ref) {
  ...
  function handleDrop(acceptedFiles, rejectedFiles, event) {
    ...
    stateReducer({
      type: 'add',
      file: [].concat(_toConsumableArray(newAcceptedFiles), _toConsumableArray(newRejectedFiles)),
  ...
  function reduceFiles(fileList) {
    ...
    var newFilesValue = fileList.reduce(function (newFiles, newFile) {
      ...
      return _extends({}, newFiles, _defineProperty({}, file.id, file));
    }, {}); // Return new file map
  ...
  function stateReducerHandler(_ref3) {
    ...
    switch (action.type) {
      ...
      case 'add':
        ...
        return {
          value: _extends({}, value, _defineProperty({}, _id, newFile)),
          action: action
        };
      // When 'remove'...
      ...
      case 'remove':
        ...
        var _action$id = action.id,
            ...
            newValueRemove = _objectWithoutProperties(value, [_action$id].map(_toPropertyKey)); // Return file map without file
      ...
      case 'modify':
        ...
        if (Array.isArray(action.file)) {
          ...
          return {
            value: _extends({}, value, action.file.reduce(function (modifiedState, modifiedFile) {
