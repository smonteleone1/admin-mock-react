...
var useListItems = function useListItems(_ref) {
  ...
  var times = optionsLabel ? [{
    ...
    value: '',
    disabled: true
  }].concat((0, _toConsumableArray2["default"])(optionsProp)) : optionsProp;
  return times.map(function (time, index) {
    var item = {};

    if ((0, _types.isParsableDateRange)(time)) {
    ...
    } else if ((0, _types.isParsableDateObject)(time)) {
    ...
    return {
      key: index,
      value: item.value,
      disabled: (0, _types.isParsableDateObject)(time) || (0, _types.isParsableDateRange)(time) ? time.disabled : false,
